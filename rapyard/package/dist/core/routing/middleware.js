...
export async function handleMiddleware(internalEvent) {
    ...
    if (internalEvent.headers["x-isr"])
        return internalEvent;
    ...
    */
    ...
    if (statusCode >= 300 && statusCode < 400) {
        resHeaders.location =
            responseHeaders
                .get("location")
                ?.replace("http://localhost:3000", `https://${internalEvent.headers.host}`) ?? resHeaders.location;
