...
import { copyFileSync, existsSync, mkdirSync, readdirSync, readFileSync, readlinkSync, statSync, symlinkSync, writeFileSync, } from "fs";
...
--------------------------------------------------------------------------------`);
    ...
    readdirSync(standaloneNextDir).forEach((f) => {
        if (statSync(path.join(standaloneNextDir, f)).isDirectory())
            return;
    ...
    readdirSync(path.join(standaloneNextDir, "server")).forEach((f) => {
        if (statSync(path.join(standaloneNextDir, "server", f)).isDirectory())
            return;
        if (f !== "server.js") {
            copyFileSync(path.join(standaloneNextDir, "server", f), path.join(path.join(outputNextDir, "server"), f));
        }
    });
    // Copy patch file
