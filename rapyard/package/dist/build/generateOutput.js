...
export async function generateOutput(outputPath, config) {
    const edgeFunctions = {};
    ...
    const requiredServerFiles = JSON.parse(fs.readFileSync(path.join(outputPath, ".next", "required-server-files.json"), "utf-8")).config;
    ...
    fs.writeFileSync(path.join(outputPath, ".open-next", "open-next.output.json"), JSON.stringify(output));
}
