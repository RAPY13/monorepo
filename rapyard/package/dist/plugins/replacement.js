            ...
            build.onLoad({ filter: target }, async (args) => {
                ...
                await Promise.all([
                    ...(deletes ?? []).map(async (id) => {
                        ...
                        logger.debug(chalk.blue(`Open-next replacement plugin ${name}`), ` -- Deleting override for ${id}`);
                    ...
                    ...(replacements ?? []).map(async (fp) => {
                        ...
                        for (const match of matches) {
                            ...
                            const id = match[1];
                            const pattern = new RegExp(`\/\/#override (${id})\n([\\s\\S]*?)\/\/#endOverride`, "g");
                            logger.debug(chalk.blue(`Open-next replacement plugin ${name}`), `-- Applying override for ${id} from ${fp}`);
