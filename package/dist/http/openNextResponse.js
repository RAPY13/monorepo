...
export class OpenNextNodeResponse extends Transform {
    ...
    constructor(fixHeaders, onEnd, streamCreator, initialHeaders) {
        ...
        if (initialHeaders && initialHeaders[SET_COOKIE_HEADER]) {
            this._cookies = parseCookies(initialHeaders[SET_COOKIE_HEADER]);
        }
        this.once("finish", () => {
            if (!this.headersSent) {
                this.flushHeaders();
    ...
    get finished() {
        return Boolean(this.writableFinished && this.responseStream?.writableFinished);
    }
