...
const incrementalCache = {
    async get(key, isFetch) {
        const result = await awsFetch(`https://${CACHE_BUCKET_NAME}.s3.${CACHE_BUCKET_REGION}.amazonaws.com/${buildS3Key(key, isFetch ? "fetch" : "cache")}`, {
            method: "GET",
        });
    ...
    async set(key, value, isFetch) {
        const response = await awsFetch(`https://${CACHE_BUCKET_NAME}.s3.${CACHE_BUCKET_REGION}.amazonaws.com/${buildS3Key(key, isFetch ? "fetch" : "cache")}`, {
            method: "PUT",
    ...
    async delete(key) {
        const response = await awsFetch(`https://${CACHE_BUCKET_NAME}.s3.${CACHE_BUCKET_REGION}.amazonaws.com/${buildS3Key(key, "cache")}`, {
            method: "DELETE",
        });
