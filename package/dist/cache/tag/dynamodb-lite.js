...
import { getDynamoBatchWriteCommandConcurrency, MAX_DYNAMO_BATCH_WRITE_ITEM_COUNT, } from "./constants";
...
const tagCache = {
    async getByPath(path) {
        try {
            if (globalThis.disableDynamoDBCache)
                return [];
            const result = await awsFetch(`https://dynamodb.${CACHE_BUCKET_REGION}.amazonaws.com`, {
                method: "POST",
                headers: {
        ...
        try {
            ...
            for (const paramsChunk of toInsert) {
                await Promise.all(paramsChunk.map(async (params) => {
                    const response = await awsFetch(`https://dynamodb.${CACHE_BUCKET_REGION}.amazonaws.com`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/x-amz-json-1.0",
