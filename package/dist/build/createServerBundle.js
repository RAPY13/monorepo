...
import { compareSemver, copyEnvFile, copyOpenNextConfig, esbuildAsync, traverseFiles, } from "./helper.js";
const require = topLevelCreateRequire(import.meta.url);
const __dirname = url.fileURLToPath(new URL(".", import.meta.url));
export async function createServerBundle(config, options) {
...
async function generateBundle(name, config, options, fnOptions) {
    ...
    fs.copyFileSync(path.join(outputDir, ".build", `cache.${ext}`), path.join(outputPath, packagePath, "cache.cjs"));
    ...
    if (!config.middleware?.external &&
        ...
        fs.copyFileSync(path.join(outputDir, ".build", "middleware.mjs"), path.join(outputPath, packagePath, "middleware.mjs"));
    }
    // Copy open-next.config.mjs
    copyOpenNextConfig(path.join(outputDir, ".build"), path.join(outputPath, packagePath));
    //Copy env files
    ...
    //       Next.js app.
    const disableNextPrebundledReact = compareSemver(options.nextVersion, "13.5.1") >= 0 ||
    ...
    const plugins = [
        openNextReplacementPlugin({
            ...
            replacements: disableRouting
                ? [
                    require.resolve("../adapters/plugins/without-routing/requestHandler.js"),
                ]
                : [],
        }),
        openNextReplacementPlugin({
...
function addMonorepoEntrypoint(outputPath, packagePath) {
    ...
    fs.writeFileSync(path.join(outputPath, "index.mjs"), [`export * from "./${packagePosixPath}/index.mjs";`].join(""));
}
