...
import { compileOpenNextConfigEdge, compileOpenNextConfigNode, } from "./build/compileConfig.js";
...
import { compareSemver, copyOpenNextConfig, esbuildAsync, esbuildSync, getBuildId, getHtmlPages, normalizeOptions, removeFiles, traverseFiles, } from "./build/helper.js";
...
export async function build(openNextConfigPath, nodeExternals) {
    ...
    if (!config || !config.default) {
        logger.error(`config.default cannot be empty, it should be at least {}, see more info here: https://open-next.js.org/config#configuration-file`);
        process.exit(1);
    }
...
function initOutputDir(tempDir) {
    ...
    if (openNextConfigEdge) {
        fs.writeFileSync(path.join(lTempDir, "open-next.config.edge.mjs"), openNextConfigEdge);
    }
}
async function createWarmerBundle(config) {
...
async function createRevalidationBundle(config) {
...
async function createImageOptimizationBundle(config) {
...
/***************************/
export function compileCache(format = "cjs") {
...
async function createMiddleware() {
    ...
    const middlewareManifest = JSON.parse(readFileSync(path.join(appBuildOutputPath, ".next/server/middleware-manifest.json"), "utf8"));
    ...
    if (config.middleware?.external) {
        ...
        copyOpenNextConfig(options.tempDir, outputPath, config.middleware.override?.wrapper === "cloudflare");
        // Bundle middleware
