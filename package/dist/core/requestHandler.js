...
export async function openNextHandler(internalEvent, responseStreaming) {
    ...
    //#endOverride
    const headers = "type" in preprocessResult
        ? preprocessResult.headers
        : preprocessResult.internalEvent.headers;
    const overwrittenResponseHeaders = Object.entries("type" in preprocessResult
        ? preprocessResult.headers
        : preprocessResult.internalEvent.headers).reduce((acc, [key, value]) => {
    ...
    if ("type" in preprocessResult) {
        ...
        const internalResult = await globalThis.__als.run({ requestId, pendingPromiseRunner, isISRRevalidation }, async () => {
            const preprocessedResult = preprocessResult;
            const req = new IncomingMessage(reqProps);
            const res = createServerResponse(preprocessedEvent, overwrittenResponseHeaders, responseStreaming);
            await processRequest(req, res, preprocessedEvent, preprocessedResult.isExternalRewrite);
...
async function processRequest(req, res, internalEvent, isExternalRewrite) {
    // @ts-ignore
