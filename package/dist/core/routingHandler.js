...
import { addNextConfigHeaders, fixDataPage, handleFallbackFalse, handleRedirects, handleRewrites, } from "./routing/matcher";
...
const optionalBasepathPrefixRegex = !!RoutesManifest.basePath
    ...
    : "^/";
const staticRegexp = RoutesManifest.routes.static.map((route) => new RegExp(route.regex
    .replace("^/", optionalLocalePrefixRegex)
    .replace("^/", optionalBasepathPrefixRegex)));
const dynamicRegexp = RoutesManifest.routes.dynamic.map((route) => new RegExp(route.regex
    .replace("^/", optionalLocalePrefixRegex)
    .replace("^/", optionalBasepathPrefixRegex)));
...
export default async function routingHandler(event) {
    ...
    const isStaticRoute = !isExternalRewrite &&
        staticRegexp.some((route) => route.test(internalEvent.rawPath));
    ...
    const isDynamicRoute = !isExternalRewrite &&
        dynamicRegexp.some((route) => route.test(internalEvent.rawPath));
    ...
    const isApiRoute = internalEvent.rawPath === "/api" ||
